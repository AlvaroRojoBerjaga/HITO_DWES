...
<%    if (request.getParameter("accion") != null) {
        ...
        try {
            String asignatura = request.getParameter("txtAsignatura");
...
%>
<div class="caja">
    <h2>Ver alumnos matriculados en una asigatura</h2>
    <div class="caja2">
        <form>
            <p>Seleccionar asignatura:</p>
            <%
                String recuperas = (String) request.getAttribute("asignatura_a");
                if (cursos_al != null) {
                    for (int i = 0; i < cursos_al.size(); i++) {
                        String p = cursos_al.get(i).getNombre();
                        if (p.equalsIgnoreCase(recuperas)) {

            %>

            <input type="radio" name="txtAsignatura"  value="<%=p%>" checked><label><%=p%></label>

            <%} else {
            %>
            <input type="radio" name="txtAsignatura"  value="<%=p%>" ><label><%=p%></label>
